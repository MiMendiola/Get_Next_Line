...
/*   Updated: 2023/10/31 12:50:26 by mmendiol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*main_str[OPEN_MAX];
	char		*print;

	print = NULL;
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
	{
		free_all(&main_str[fd]);
		return (NULL);
	}
	main_str[fd] = read_till_find(fd, main_str[fd]);
	if (!main_str[fd])
	{
		free_all(&main_str[fd]);
		return (NULL);
	}
	relocate_line(&main_str[fd], &print);
	return (print);
}
